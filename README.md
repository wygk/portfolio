...
> 专注测试测量上位机、高速数据采集、多线程实时处理、多协议通信与后端微服务架构
...
- 🎯 **求职意向**：自动化测试系统 / 测试测量上位机研发工程师（期望城市：武汉）
- 🧩 **核心方向**：上位机开发 · 高速数据采集 · 多线程实时处理 · Socket 网络通信 · 多协议通信驱动 · 数字孪生可视化 · 微服务架构
- 🤖 **AI 辅助研发**：熟练使用 AI-Agent 工具（Copilot/Cursor/通义灵码等）进行编码、单元测试与缺陷分析，已融入日常开发流程
...
`基于 Socket 的 TCP/IP · UDP（自定义协议、粘包拆包、心跳重连）` · `HTTP/WebAPI` · `串口` · `Modbus RTU/TCP/UDP` · `CAN / CANFD（周立功、广成）` · `PLC（西门子 S7）` · `熟悉 EtherCAT 总线架构`
...
**其他**：多线程（线程安全/线程同步/并发集合/async-await） · 委托 / 反射 / 队列 / 内存管理 · AI-Agent 辅助编码与单元测试 · Apollo 配置中心 · SkyWalking 链路追踪 · ELK
...
- 基于 Socket 实现 TCP/UDP 设备通信：自定义帧协议解析、粘包拆包处理、断线心跳重连，保障设备 7×24 小时连续采集稳定性
- 对接 PLC（西门子 S7）读写与工业相机/摄像头图像采集，集成 OpenCV 实现二维码识别，实现设备参数配置、实时监控与联动
- 实现生产者-消费者多线程采集架构：采集、解析、存储、显示四级流水线线程隔离，通过有界队列+线程同步原语保障线程安全，解决高速数据场景下的零丢包与实时刷新问题
- 开发数据后处理模块：波形回放、数据筛选、异常标记与统计分析报表导出，显著提升测试效率
- 封装 C++ 高性能信号处理算法，通过 P/Invoke 集成至 C# 主程序，处理性能提升 60%+
- 结合 AI-Agent 工具完成核心模块单元测试编写与疑难缺陷分析，缩短问题定位与回归验证周期
